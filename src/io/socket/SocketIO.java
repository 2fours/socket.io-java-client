...
public class SocketIO {
	...
	/**
	 * Instantiates a new socket.io connection. The object connects after calling {@link #connect(URL, IOCallback)} or
	...
	/**
	 * Instantiates a new socket.io connection and sets the request headers used while connecting the first time for authorizing. The object
	 ...
	 *             the malformed url exception
	 */
	public SocketIO(final String url, Map<String, String> headers) throws MalformedURLException {
		if (url == null)
			throw new RuntimeException("url may not be null.");
	...
	/**
	 * connects to an already set host. Do only use this method if you instantiate {@link SocketIO} using {@link #SocketIO(String)} or
	 * {@link #SocketIO(URL)}.
	 * 
	 * @param callback
	 *            the callback
	...
	/**
	 * Emits an event to the Socket.IO server. If the connection is not established, the call will be buffered and sent as soon as it is
	 * possible.
	 * 
	 * @param event
	 *            the event name
	 ...
	 * 
	 * This had become useful on some android devices which do not shut down tcp-connections when switching from HSDPA to Wifi
	 */
	...
	/**
	 * Returns the headers used while handshaking. These Properties are not necessarily the ones set by {@link #addHeader(String, String)}
	 * or {@link #SocketIO(String, Properties)} but the ones used for the handshake.
	 * 
	...
	/**
	 * Sets the headers used while handshaking. Internally used. Use {@link #SocketIO(String, Properties)} or
